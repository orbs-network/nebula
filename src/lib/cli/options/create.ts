...
export function createOptions(yargs: Argv) {
  return yargs
    .option("file", {
      describe:
    ...
    .option("name", {
      describe:
        "name your constellation! in case non supplied defaults to a random name",
      default: ""
    })
    .option("aws-profile", {
      describe:
        "which aws profile name to use when provisioning. Strongly recommended instead of AWS keys for better security",
      default: "default"
    })
    .boolean("testnet")
    .boolean("no-ethereum")
    .option("public-ip", {
      describe:
        "attach a pre-existing AWS Elastic IP to the provisioned constellation",
      default: false
    })
    .option("orbs-address", {
    ...
    .option("node-count", {
      describe:
        "The amount of worker nodes within your constellation (this nodes carry the load of operating virtual chains)",
      default: 2
    })
    .option("node-size", {
    ...
    .option("ssh-public-key", {
      describe:
        "The path to the public key used to provision the constellation machines",
