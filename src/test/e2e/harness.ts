...
import fixtures from "./fixtures/nodes.json";
import boyar from "./fixtures/boyar.json";
...
export default {
  ...
  deleteNodesJSONsFromDisk(jsons) {
    return Promise.all(
      jsons.map((_, index) =>
        unlink(
          path.join(__dirname, "private-network/nodes", `node${index + 1}.json`)
        )
      )
    );
  },
  ...
  writeBoyarConfig() {
    const targetPath = path.join(
      __dirname,
      "private-network/templates/boyar.json"
    );
  ...
  writeConfigurationFiles(nodes) {
    ...
    fs.writeFileSync(
      ipsTargetPath,
      JSON.stringify(generateIpsConfig(nodes), 2, 2)
    );
    fs.writeFileSync(
      keysTargetPath,
      JSON.stringify(generateKeysConfig(nodes), 2, 2)
    );
  },
  aws: {
  ...
  getNodesJSONs(
    { elasticIPs, buildNumber = circleCiBuildNumber() },
    ...
    return nodes.map(node => {
      ...
      const { ip: publicIp } = elasticIPs.filter(
  ...
  async eventuallyReady({ ip, boyar, address }) {
    ...
    while (poll && pollCount < 60) {
      try {
        ...
        const boyarCheck = await exec(
          `ssh -o StrictHostKeyChecking=no ubuntu@${ip} 'test -e /usr/bin/boyar'`
        );
        expect(boyarCheck.exitCode).to.equal(0);

        const swarmLeaderCheck = await exec(
          `ssh -o StrictHostKeyChecking=no ubuntu@${ip} 'sudo docker node ls | grep Leader | wc -l'`
        );
