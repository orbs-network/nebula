...
import { waitUntilVersion, getVersion } from "../../lib/metrics";
...
describe("nebula upgrade node version", () => {
  before(async () => {
    ...
    console.log(
      "Getting an Elastic IP in the following region: ",
      singleNode.region
    );
    ...
    console.log(
      "Got back the following IP allocation result from AWS:",
      elasticIPs
    );
    ...
    console.log(
      "********* NEBULA UPGRADE VERSION TEST GLOBAL SETUP FINISHED **********"
    );
  });
  ...
  it("should upgrade the binary version for a constellation", async () => {
    ...
    const fileObject = {
      file: path.join(__dirname, "/private-network/nodes/node1.json")
    };
  ...
  after(async () => {
    ...
    console.log(
      "********* NEBULA UPGRADE VERSION TEST GLOBAL TEARDOWN START **********"
    );
    ...
    console.log(
      "********* NEBULA UPGRADE VERSION TEST GLOBAL TEARDOWN FINISHED **********"
    );
  });
});
