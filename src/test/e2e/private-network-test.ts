...
import { waitUntilSync, getBlockHeight } from "../../lib/metrics";
import { create, destroy, update } from "../../lib/cli/cli";
...
describe("nebula setup a private network", () => {
  before(async () => {
    ...
    console.log(
      "********* NEBULA PRIVATE BLOCKCHAIN TEST GLOBAL SETUP START **********"
    );
    ...
    console.log(
      "Got back the following IP allocations results from AWS:",
      elasticIPs
    );
    ...
    console.log(
      "********* NEBULA PRIVATE BLOCKCHAIN TEST GLOBAL SETUP FINISHED **********"
    );
  });
  ...
  it("should provision a 3 node network and then join a 4th node", async () => {
    ...
    console.log(
      "********** NEBULA CREATE 3 NODES OF PRIVATE BLOCKCHAIN BEGIN ***********"
    );
    const creations = nodes.map(node => create(node).catch(err => err));
    const results = await Promise.all(creations);
    nebulaCreationStepMarker = true;
    console.log(
      "********** NEBULA CREATE 3 NODES OF PRIVATE BLOCKCHAIN END ***********"
    );

    const errornousCreations = results.filter(r => r.ok === false);
    expect(
      errornousCreations.length,
      "Expecting no failures creating the 3 nodes"
    ).to.equal(0);
    ...
    const resultNode4 = await create({
      file: path.join(basePath, "node4.json")
    }).catch(err => err);
    ...
    console.log(
      "********** NEBULA UPDATE FIRST 3 NODES TOPOLOGY BEGIN ***********"
    );

    const updateResults = await Promise.all(
      nodes.map(node => update(node).catch(err => err))
    );
    const successfulUpdates = updateResults.filter(r => r.ok === true);

    expect(
      successfulUpdates.length,
      "Expect all 3 updates to work correctly"
    ).to.equal(3);
    console.log(
      "********** NEBULA UPDATE FIRST 3 NODES TOPOLOGY END ***********"
    );
    ...
    expect(
      firstNodeBlockHeight,
      "block height should advance with 4th node added"
    ).to.be.gte(30);
    ...
    expect(
      lastNodeBlockHeight,
      "block height should advance with 4th node added"
    ).to.be.gte(firstNodeBlockHeight + 10);
  });
  ...
  after(async () => {
    ...
    if (nebulaCreationStepMarker) {
      await Promise.all(
        _.take(nodesJSONs, 3).map(node => destroy(node).catch(err => err))
      );
    }
    ...
    console.log(
      "********* NEBULA PRIVATE BLOCKCHAIN TEST GLOBAL TEARDOWN START **********"
    );
    const validElasticIPs = elasticIPs.filter(o => o.ok === true);
    console.log(
      "Releasing the following Elastic IPs from our AWS account: ",
      validElasticIPs
    );
    const elasticIPsReleaseResults = await Promise.all(
      validElasticIPs.map(({ ip, region }) =>
        harness.aws.destroyPublicIp(region, ip)
      )
    );
    ...
    console.log(
      "********* NEBULA PRIVATE BLOCKCHAIN TEST GLOBAL TEARDOWN FINISHED **********"
    );
  });
});
