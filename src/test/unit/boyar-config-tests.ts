...
import { createBoyarConfig } from "../../lib/cli/boyar/create-config";
...
describe("boyar config generation", () => {
  it("generates network configuration", () => {
    ...
    expect(
      config.chains[0].Config["benchmark-consensus-constant-leader"]
    ).to.be.eql("6e2cb55e4cbe97bf5b1e731d51cc2c285d83cbf9");
  });
  ...
  it("updates all vchains to the same version", () => {
    ...
    expect(config.chains[0].DockerConfig.Tag).to.be.eql(
      "some-random-chain-version"
    );
    expect(config.chains[1].DockerConfig.Tag).to.be.eql(
      "some-random-chain-version"
    );
    expect(config.chains[2].DockerConfig.Tag).to.be.eql(
      "some-random-chain-version"
    );
  });
