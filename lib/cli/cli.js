...
async function create(argv) {
    ...
    if (argv.file !== undefined && argv.file !== '') {
        ...
        if (!_.isEmpty(argv.orbsAddress)) {
            _argv.orbsAddress = argv.orbsAddress;
        }

        if (!_.isEmpty(argv.orbsPrivateKey)) {
...
async function _create({ fileMode = false, __fileDirname = '', awsProfile, sshPublicKey: _sshPublicKey, orbsAddress, orbsPrivateKey, region,
    nodeSize, publicIp, noEthereum = false, nodeCount, cachePath: _catchPath, name, bootstrapUrl, configPath: _configPath, chainVersion, _file, ethereumChain, ethereumSyncBucket, ethereumTopologyContractAddress, incomingSshCidrBlocks }) {
    let errorMessage;
...
async function _destroy({ name, cachePath: _catchPath, fileMode, __fileDirname }) {
    let cachePath = _catchPath;
...
async function _update({ name, region, configPath: _configPath, chainVersion, awsProfile, ethereumTopologyContractAddress, fileMode = false, __fileDirname }) {
    let configPath = _configPath;
...
async function status({ configPath, vchain }) {
    ...
    const result = await getStatus(ips, 1000, 60000);

    _.each(result, (data, name) => {
        const color = data.status == "green" ? logGreen : logRed;
        const commit = _.truncate(data.commit, { length: 8, omission: '' });
        color(`${name} ${data.status} blockHeight=${data.blockHeight} version=${data.version}@${commit} eth[blockHeight=${data.ethereum.lastBlock} sync=${data.ethereum.syncStatus} txReceipts=${data.ethereum.txReceiptsStatus}]`);
    });
...
// FIXME: support generating config for a certain node (-f parameter)
