...
async function _create({ fileMode = false, __fileDirname = '', awsProfile, sshPublicKey, orbsAddress, orbsPrivateKey, region,
    nodeSize, publicIp, noEthereum = false, nodeCount, name, configPath: _configPath, chainVersion }) {
    let errorMessage;
    let configPath = '';
...
async function _update({ name, configPath, chainVersion, awsProfile }) {
    ...
    const command = `aws s3 cp --acl public-read ${tmpPath} ${profile} s3://${boyarBucketName}/boyar/config.json`;
}
