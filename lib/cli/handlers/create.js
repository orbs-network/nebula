...
const { getWeb3Adapter } = require('./../../services/adapters/eth-adapter');
...
async function create(params) {
    ...
    const resp = await fetch('http://orbs-bootstrap-prod.s3.amazonaws.com/boyar/config.json');
    const prodBoyarConfig = await resp.json();
    prodBoyarConfig.chains.forEach(chain => {
        chain.DockerConfig.Tag = process.env.VERSION;
    });

    const result = await nebula.createConstellation({
        topologyProvider: getWeb3Adapter(),
        chains: prodBoyarConfig.chains,
        cloud,
        keys,
    });
