...
class TerraformService {
    ...
    async spinUp({ cloud, keys }) {
        ...
        const name = cloud.name || uuid();
        const target = path.join(this.tmpBasePath(), name);

        // TODO: Maybe split this into a different part of the system.
        await this.createSpinContextDirectory({ target });
        await this.writeTerraformVariablesFile({ name, cloud, keys });
        await this.copyTerraformInfraTemplate({ cloud, name });
        await this.copyStackScripts({ name });

        // First step is to init terraform
        const initResult = await this.init({ name });
