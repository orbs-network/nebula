...
function serializeKeys(keys) {
    return Buffer.from(JSON.stringify({
        "node-public-key": keys.publicKey,
...
class TerraformService {
    ...
    createTerraformVariablesFile({ cloud, keys }) {
        ...
        if (cloud.type === types.clouds.aws) {
            ...
            contentAsString += `node_key_pair = "${serializeKeys(keys.orbs)}"`
        }
...
=======
        await this.init({ spinContext: spinContext });
        await this.destroy({ spinContext });
>>>>>>> 49ffc2dd848ae913da6256a33300036f17d1c8b2
