...
const { trim, isError, isEmpty, isInteger } = require('lodash');
...
class Terraform {
    ...
    setCachePath(cachePath) {
        this.cachePath = this.resolveHome(cachePath);
    }

    setTargetPath(name) {
        this.targetPath = path.join(this.terraformBasePath(), name);
    }
    ...
    terraformBasePath() {
        ...
        return this.resolveHome(defaultCachePath);
    }
    ...
    createTerraformVariablesFile({ cloud, keys }) {
        ...
        if (!isEmpty(keys.ssh.cidr)) {
            contentAsString += `incoming_ssh_cidr_blocks=${JSON.stringify(keys.ssh.cidr)}\n`;
        }
        ...
        contentAsString += `name = "${cloud.name}"\n`;

        // AWS Credentials
        contentAsString += `aws_profile = "${keys.aws.profile}"\n`;
        contentAsString += `region = "${cloud.region}"\n`;

        contentAsString += `instance_type = "${cloud.instanceType}"\n`;
        contentAsString += `instance_count = "${isInteger(cloud.nodeCount) ? cloud.nodeCount : 2}"\n`;
        ...
        if (!isEmpty(keys.orbs.ethereumEndpoint)) {
            contentAsString += `ethereum_endpoint = "${keys.orbs.ethereumEndpoint}"\n`;
        }

        if (!isEmpty(keys.orbs.ethereumChain)) {
            contentAsString += `ethereum_chain = "${keys.orbs.ethereumChain}"\n`;
        }

        if (!isEmpty(keys.orbs.ethereumTopologyContractAddress)) {
            contentAsString += `ethereum_topology_contract_address = "${keys.orbs.ethereumTopologyContractAddress}"\n`;
        }
    ...
    async create({ cloud, keys }) {
        ...
        try {
            ...
            if (eip) {
                outputs[outputs.findIndex(o => o.key === 'manager.ip')].value = cloud.ip;
            }
