...
class Terraform {
    ...
    createTerraformVariablesFile({ cloud, keys, chains }) {
        const rows = [];
    ...
    async writeTerraformVariablesFile({ cloud, keys, chains }) {
        const contentAsString = this.createTerraformVariablesFile({ cloud, keys, chains });
        const target = path.join(this.targetPath, 'terraform.tfvars');

        await writeFile(target, contentAsString);
    }

    async create({ cloud, keys, chains }) {
        ...
        try {
            ...
            await this.writeTerraformVariablesFile({ cloud, keys, chains });
