...
const terraform_1 = require("../../lib/services/terraform");
const tf = new terraform_1.Terraform();
mocha_1.describe('terraform service unit tests', () => {
    mocha_1.it('should parse terraform outputs correctly', () => {
        const cleanOutputs = [
            {
    ...
    mocha_1.it('should return an empty [] when there are no outputs in the string', () => {
        const result = tf.parseOutputs('fadssaf\n');
        expect(result).to.be.array();
        expect(result.length).to.equal(0);
    });
    mocha_1.describe("createTerraformVariablesFile", () => {
        mocha_1.it("default call", () => {
            const keys = {
                "aws": {
                    "profile": "default"
                },
            ...
            expect(tf.createTerraformVariablesFile({ keys, cloud })).to.equal(`path_to_ssh_pubkey = "${process.env.HOME}/.ssh/id_rsa.pub"
...
//# sourceMappingURL=terraform-tests.js.map